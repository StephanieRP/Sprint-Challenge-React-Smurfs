...
Biggest difference between the two, client side routing does not force the entire page to refresh and download the content when navigating though the application. While server-side refreshes the entire page but is best for SEO.

- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.
      Create, Read, Update and delete. The methods are get, post, put and delete. We use these to interact with the data that is on the server.
- [ ] Mention three tools we can use to make AJAX requests.
      We use fetch, then and catch to request data by promises that are done by fetch api's
